...
class Message(object):
    ...
    def apply_shift(self, shift):
        ...
        for l in self.message_text:
            if l in s_all:
                self.new_dic = self.build_shift_dict(shift)
                self.new_s += self.new_dic[l]
            else:
                self.new_s += l
        return self.new_s
        ...
        for i in range(26):
            for j in list(super(CiphertextMessage, self).apply_shift(i).split(' ')):
                if is_word(self.valid_words, j):
                    real_word += 1
                if real_word > max_count:
                    max_count = real_word
                    shift_best = i
                    decrypted_msg = super(CiphertextMessage, self).apply_shift(i)
                        
        return (shift_best, decrypted_msg)
    
    def decrypt_a_story(self):
...
print('story: ',ciphertext.decrypt_a_story())
